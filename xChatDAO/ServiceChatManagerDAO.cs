...
namespace xChatDAO
{
    ...
    public class ServiceChatManagerDAO : IServiceChatManagerDAO
    {
        ...
        public ObjectResultList<UserConnect> GetListUserConnectByAccountManagerId(ObjectRequest<int> objectRequest)
        {
            ...
            try
            {
                ObjCmd = new SqlCommand("chat.AccountManager_GetListUserConnect_sp", clsConnection.GetConnection());
        ...
        public ObjectResultList<AccountManagerConnect> GetAccountManagerById(ObjectRequest<string> objectRequest)
        {
            ...
            try
            {
                ObjCmd = new SqlCommand("chat.AccountManagerConnect_GetById_Sp", clsConnection.GetConnection());
                ObjCmd.CommandType = CommandType.StoredProcedure;

                ObjCmd.Parameters.AddWithValue("@userid", objectRequest.SenderObject.Split(';')[0].ToString());
                ObjCmd.Parameters.AddWithValue("@moduleappid", objectRequest.SenderObject.Split(';')[1].ToString());
                ObjCmd.Parameters.AddWithValue("@rolid", objectRequest.SenderObject.Split(';')[2].ToString());
